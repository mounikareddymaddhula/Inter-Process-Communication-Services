...
all: scmd verify sgverify libsnappyc.so.1 server app1

snappy.o: snappy.c compat.h snappy-int.h

scmd: scmd.o snappy.o map.o util.o

CLEAN := scmd.o snappy.o scmd bench bench.o fuzzer.o fuzzer map.o verify.o client.o \
	 verify util.o sgverify sgverify.o snappy.html snappy.man libsnappyc.so.1 server app1

clean: 
	rm -f ${CLEAN}

server: server.c
	gcc -o server server.c snappy.o

client.o: client.c
	gcc -pthread -c client.c

app1: app1.c client.o
	gcc -pthread -o app1 app1.c client.o
